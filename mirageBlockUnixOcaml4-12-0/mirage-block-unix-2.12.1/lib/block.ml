                      ...
                      Lwt_list.iter_s (fun buffers ->
                        let rec loop remaining =
                          if Cstructs.len remaining = 0 then Lwt.return_unit else begin
                            ...
                            Lwt_unix.run_job ~async_method:Async_none (Raw.readv_job (Lwt_unix.unix_file_descr fd) iovec)
                            >>= fun n ->
                  ...
                  ( if is_win32 || List.length buffers = 1 then begin
                      ...
                      Lwt_list.iter_s (fun buffers ->
                        let rec loop remaining =
                          if Cstructs.len remaining = 0 then Lwt.return_unit else begin
                            let iovec = Cstructs.to_iovec remaining in
                            Lwt_unix.run_job ~async_method:Async_none (Raw.writev_job (Lwt_unix.unix_file_descr fd) iovec)
                            >>= fun n ->
                            loop (Cstructs.shift remaining n)
                          end in
                        loop buffers
