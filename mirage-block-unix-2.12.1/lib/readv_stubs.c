...
static void fill_iovecs (struct iovec* iovecs, value iovec_list, int count)
{
  CAMLparam1 (iovec_list);
  CAMLlocal5(next, head, val_buf, val_ofs, val_len);
  ...
  for (i = 0; i < count; i++) {
    ...
    val_ofs = Field(head, 1);
    val_len = Field(head, 2);
    iovecs[i].iov_base = (char*)Caml_ba_data_val(val_buf) + Long_val(val_ofs);
    iovecs[i].iov_len = Long_val(val_len);
    next = Field(next, 1);
  }
...
CAMLprim value aeio_unix_bytes_readv (value fd, value iovec_list)
{
  CAMLparam2(fd, iovec_list);
  CAMLlocal1(next);
  int length = 0;
  for (next = iovec_list; next != Val_emptylist; next = Field(next, 1))
    length++;
  /* Only copy up to the iovec array size */
  if (length > IOV_MAX)
    length = IOV_MAX;
 ...
 // printf("\nCOUNT : %d", count);
  ...
  fill_iovecs(iovecs, iovec_list, count);
  //printf("\n Fd : %d", Int_val(fd));
  ssize_t bytes_read = readv (Int_val(fd), iovecs, count);
   //printf("\n Number of bytes read %u", bytes_read);
  //for(int i = 0; i < count; i++)
    //printf("\n Iovecs %s :", (char *)iovecs[i].iov_base);
    
  if (bytes_read < 1)
    uerror ("readv", Nothing);

  CAMLreturn(Val_long(bytes_read));
}